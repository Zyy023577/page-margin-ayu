...
export async function startServer({ production = false, port = Number(process.env.PORT || 5173), desktop = false } = {}) {
  ...
  app.use((req, res, next) => {
    if (!originAllowed(req.headers.origin)) return res.status(403).json({ error: '该网站来源不允许访问此接口。' });
  ...
  app.get('/api/status', (_req, res) => {
    ...
    res.json({ mode: provider.name === 'demo' ? 'demo' : 'live', provider: provider.name, quota: budget.snapshot() });
  });
  app.post('/api/ai', apiLimiter, async (req, res) => {
    const validation = validateAiRequest(req.body);
    if (!validation.ok) return res.status(400).json({ error: validation.error });
    const provider = createProvider(process.env, req.query.provider === 'demo');
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(new Error('生成超时')), Number(process.env.AI_TIMEOUT_MS || 45000));
    let finished = false;
    if (provider.name !== 'demo') {
      const reservation = budget.reserve(budget.estimate(validation.value, 700));
      if (!reservation.ok) {
        clearTimeout(timeout);
        return res.status(429).json({ error: reservation.error });
      }
    }
    ...
    try {
      writeEvent(res, 'meta', { provider: provider.name, demo: provider.name === 'demo' });
      for await (const event of provider.streamReply(validation.value, { signal: controller.signal })) {
        if (event.type === 'delta') writeEvent(res, 'delta', { text: event.text });
        else if (event.type === 'result') writeEvent(res, 'done', { ...event.result, demo: provider.name === 'demo' });
      }
      finished = true;
      res.end();
    } catch (error) {
      if (!res.destroyed) {
        writeEvent(res, 'error', { error: controller.signal.aborted ? (controller.signal.reason?.message || '生成已取消') : `AI 暂时没有回应：${error.message}`, retryable: true });
        res.end();
      }
    } finally {
      finished = true;
  ...
  app.post('/api/memory', apiLimiter, async (req, res) => {
    ...
    const reservation = budget.reserve(budget.estimate(validation.value, 700));
    if (!reservation.ok) return res.status(429).json({ error: reservation.error });
    ...
    try {
      const upstream = await fetch(`${base}/chat/completions`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${memoryKey}` }, body: JSON.stringify({ model: process.env.AI_MODEL || 'gpt-5.4-mini', messages: [{ role: 'system', content: system }, { role: 'user', content: prompt }], temperature: 0.2, max_tokens: 700, response_format: { type: 'json_object' } }) });
      if (!upstream.ok) throw new Error(`上游服务返回 ${upstream.status}`);
      const data = await upstream.json();
      const raw = data.choices?.[0]?.message?.content || '{}';
      const parsed = JSON.parse(raw.replace(/^```json\s*|\s*```$/g, ''));
      return res.json({ demo: false, memory: normalizeMemoryResponse(parsed) });
    } catch (error) {
      return res.status(502).json({ error: `章节记忆生成失败：${error.message}` });
    }
  });
  ...
  console.log(`页边的阿屿已启动：http://127.0.0.1:${actualPort}`);
  return { app, server: listener, port: actualPort };
}

const isMain = process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url);
if (isMain) await startServer({ production: process.argv.includes('--production') });
