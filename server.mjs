...
app.post('/api/ai', async (req, res) => {
  ...
  if (!process.env.AI_API_KEY) return res.json({ demo: true, content: demoReply(action, selectedText, style) });
...
function demoReply(action, text, style) {
  ...
  return tone + (variants[action] || variants.ask);
}
