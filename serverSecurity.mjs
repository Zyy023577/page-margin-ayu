...
export function createOriginChecker(env=process.env){const configured=String(env.ALLOWED_ORIGINS||'').split(',').map(value=>value.trim().replace(/\/$/,'')).filter(Boolean);const development=env.NODE_ENV!=='production';return origin=>{if(!origin)return true;const normalized=origin.replace(/\/$/,'');if(configured.includes(normalized))return true;if(development&&/^http:\/\/(localhost|127\.0\.0\.1)(:\d+)?$/.test(normalized))return true;return false;};}
